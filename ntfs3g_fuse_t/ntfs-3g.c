...
static int ntfs_context_init(void)
{
...
int start(int argc, char *argv[], BOOL hide_hid_files)
{
	int err = 0;
	ntfs_set_locale();

	if (ntfs_parse_options(&opts, argc, argv)) {
		return NTFS_VOLUME_SYNTAX_ERROR;
	}

	if (ntfs_context_init()) {
		err = NTFS_VOLUME_OUT_OF_MEMORY;
		goto err2;
	}

	char* parsed_options = parse_mount_options(ctx, &opts, FALSE, hide_hid_files);
	if (!parsed_options) {
    ...
    } else if (ctx->rw && ntfs_strinsert(&parsed_options, ",rw")) {
        goto err_out;
    }
    ...
    struct fuse *fh = mount_fuse(parsed_options);
	if (!fh) {
...
int main(int argc, char *argv[]) {
    setenv("FUSE_NFSSRV_PATH", argv[3], 1);
    BOOL hide_hid_files = !strcmp(argv[4], "1");
    return start(argc, argv, hide_hid_files);
}
