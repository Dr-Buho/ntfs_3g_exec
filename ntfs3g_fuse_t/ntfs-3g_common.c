...
char *parse_mount_options(ntfs_fuse_context_t *ctx,
			const struct ntfs_options *popts, BOOL low_fuse, BOOL hide_hid_files)
{
    ...
    ctx->hide_hid_files = hide_hid_files;
exit:
	free(options);
	return ret;
err_exit:
 ...
 */
int ntfs_parse_options(struct ntfs_options *popts, int argc, char *argv[])
{
	int c;
